...
export const EditProfileModal: FC<EditProfileModalPropsType> = (
   { isOpen, onClose, onSubmit, fullName, contacts, aboutMe, onChangeFullName, onChangeAboutMe, onChangeContacts }
) => {
   ...
   const validate = (data: { [key: string]: { value: string, errorHandler: (isError: boolean) => void } },
      ...
      >
         <div className='edit-profile-modal'>
            <div className='edit-profile-modal__body'>
               <TextField label={isNameError ? 'Обязательно' : 'Имя'}
                  variant="standard"
                  value={fullName}
            ...
            <div className='edit-profile-modal__buttons'>
               <Button color='error' variant='outlined' onClick={onCancelHandler}>Отменить</Button>
               <Button color='success' variant='outlined' onClick={onSubmitHandler}>Сохранить</Button>
            </div>
         </div>
      </Modal>
   )
